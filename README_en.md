...
> [!IMPORTANT]
> Chinese pronunciation uses `pypinyin` and `pypinyin-dict`, supplemented with [CC-CEDICT](https://cc-cedict.org/) and project-level phrase pronunciations from `data/phrases.json`. Dictionaries load in the order base data → CC-CEDICT/di → project corrections. Other pronunciation-based schemes use the same data.
>
> Chinese word segmentation uses `jieba` and `data/dict.txt`. Pinyin-only orthographic boundary corrections live in `data/pinyin_splits.json`, so they do not alter the other transcription schemes. The pipeline then refines ambiguous phrase boundaries, joins syllables by word, and inserts apostrophes before a-, o-, and e-initial syllables where required.
>
> Orthography focuses on rules directly exercised by the current Minecraft corpus: word-based spacing (5.1–5.5), aspect particles and common syntactic boundaries (6.1.2), demonstrative and classifier spacing (6.1.4), numeral/classifier spacing and ordinal hyphens (6.1.5), 2+2 hyphens for four-character idioms that actually occur (6.1.12), apostrophes (6.2), sentence capitalization (6.3), unmarked neutral tones and the original tones of “一” and “不” (6.5), and Latin punctuation (6.6). Minecraft formatting codes such as `§a` and placeholders such as `%s`, `%1$s`, and `%%` are preserved byte-for-byte.
>
> “Reviewed against the standard” is not a claim of formal certification. Automatic segmentation cannot infer every grammatical relation, proper noun, idiom structure, or polyphonic reading from context. The project maintains only vocabulary and corrections required by actual Minecraft language entries. Corrections should be added to `data/phrases.json`, `data/dict.txt`, `data/pinyin_splits.json`, or the language-key fix data instead of adding unrelated general vocabulary or hard-coded language keys.
